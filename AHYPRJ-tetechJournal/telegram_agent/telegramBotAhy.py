...
class TelegramBot:
    ...
    async def handle_stories(self, update: Update, context: CallbackContext,  file_name, caption):
        ...
        # ✅ Executa autopost.py normalmente
        autopost_script = os.path.abspath(os.path.join(
            "..", "agent_social_media", "autopost.py"))
        # Define a pasta de execução correta
        working_dir = os.path.abspath(os.path.join("..", "agent_social_media"))
        print(
            f"Executando HandleStories: {sys.executable} {autopost_script} com cwd={working_dir}")
        subprocess.Popen([sys.executable, autopost_script], cwd=working_dir)
        
        subprocess.Popen([sys.executable, autopost_script], cwd=working_dir)
        # subprocess.Popen(["py", autopost_script], cwd=working_dir, shell=True)  # ✅ Agora ele roda na pasta correta
    ...
    async def handle_posts(self,  update: Update, context: CallbackContext, file_name, caption):
        ...
        # Cria o arquivo `caption.txt` com o texto após "/stories"
        ...
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text=f"Imagem e descrição salvas"
        )
        ...
        print(
            f"Executando HandlePost: {sys.executable} {autopost_script} com cwd={working_dir}")
        subprocess.Popen([sys.executable, autopost_script], cwd=working_dir)
        # subprocess.Popen(["py", autopost_script], cwd=working_dir, shell=True)  # ✅ Agora ele roda na pasta correta
