...
class TaskScheduled:
    ...
    def get_python_executable():
        """Obtém o caminho real do executável Python."""
        try:
            if platform.system() == "Windows":
                # Primeiro tenta localizar o py.exe, que é seguro
                py_launcher = shutil.which('py')
                if py_launcher and os.path.exists(py_launcher):
                    return py_launcher

                # Depois tenta localizar python.exe
                python_executable = subprocess.check_output(['where', 'python'], text=True).strip().split('\n')[0]

                # Evitar pegar WindowsApps (atalho falso)
                if "WindowsApps" in python_executable:
                    raise EnvironmentError(
                        "Python encontrado é um atalho da Microsoft Store. "
                        "Instale o Python real do python.org."
                    )
                return os.path.abspath(python_executable)
            else:
                # Linux/macOS: normal
            ...
            raise EnvironmentError("Não foi possível localizar um executável válido do Python.") from e
