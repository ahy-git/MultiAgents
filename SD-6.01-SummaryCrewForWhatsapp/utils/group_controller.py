...
from utils.receiveWhatsapp import MessageWhatsapp
...
class GroupController:
    def __init__(self):
        ...
        self.cache_file = os.path.join(os.path.dirname(__file__),"group_cache.json")
    ...
    def _is_cache_valid(self):
        """
        Verifica se ja faz mais de cache_ttl_minutes desde a ultima chamada do fetch_all_groups
        ...
        try:
            with open(self.cache_file,"r",encoding="utf-8") as f:
                data = json.load(f)
            ts = datetime.fromisoformat(data.get("_timestamp"))
            return (datetime.now() - ts) < timedelta(minutes=self.cache_ttl_minutes)
        except Exception:
            return False
    
    def _load_from_cache(self):
        "Carrega groups a partir do Json Local Cache"
        with open(self.cache_file,"r",encoding="utf-8") as f:
            data = json.load(f)
            return data["groups"]
    
    def _save_to_cache(self, groups_data):
        """ Salva o json(local cache) com os dados dos grupos """
    ...
    def fetch_groups(self, force_refresh=False):
        ...
        if not force_refresh and self._is_cache_valid():
            groups_data = self._load_from_cache()
        else:              
            ...
            self._save_to_cache(groups_data)
    ...
    def get_messages(self, group_id, start_date, end_date):
        ...
        msgs = MessageWhatsapp.get_messages(group_mensagens)
